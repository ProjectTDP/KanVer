...
class _CreateRequestScreenState extends ConsumerState<CreateRequestScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Hastaneler yüklenemedi: $e')));
      }
    } finally {
  ...
  Future<void> _submit() async {
    if (!_formKey.currentState!.validate() ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Lütfen tüm alanları doldurun ve bir hastane seçin.'),
        ),
      );
      return;
    }
    ...
    if (_currentPosition == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Konum bilgisi alınamadı. Lütfen konumu etkinleştirin.',
          ),
        ),
      );
      return;
    ...
    try {
      final data = {
        'hospital_id':
            _selectedHospital!['hospital_id'] ?? _selectedHospital!['id'],
        ...
        'latitude': _currentPosition!.latitude,
        'longitude': _currentPosition!.longitude,
        'patient_name': _patientNameController.text.isEmpty
            ? null
          ...
          child: Column(
            ...
            children: [
              ...
              if (_isLoadingHospitals)
                ...
                const Text(
                  'Yakında hastane bulunamadı. Lütfen hastane yakınında olduğunuzdan emin olun.',
                ...
                DropdownButtonFormField<Map<String, dynamic>>(
                  ...
                  items: _nearbyHospitals.map((h) {
                    return DropdownMenuItem<Map<String, dynamic>>(
                      value: h,
                      child: Text(
                        h['hospital_name'] ?? h['name'] ?? 'Bilinmeyen Hastane',
                      ),
                    );
                  }).toList(),
  ...
  Widget _buildPriorityChip(String value, IconData icon) {
    ...
    final color = value == 'CRITICAL'
        ? Colors.red
        : (value == 'URGENT' ? Colors.orange : Colors.blue);
